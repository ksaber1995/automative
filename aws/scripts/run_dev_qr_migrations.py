"""
Runs migrations 028 (companies.type) and 029 (students.qr_token) against the
DEV Aurora cluster via the RDS Data API. Idempotent — safe to re-run.
Usage: python scripts/run_dev_qr_migrations.py
"""
import boto3
import re
import os
...
# Dev cluster + secret (from migrate_027_monthly_subscriptions.py)
...
FILES = [
    os.path.join(SQL_DIR, 'migrations', '028_add_company_type.sql'),
    os.path.join(SQL_DIR, 'migrations', '029_student_qr.sql'),
]
